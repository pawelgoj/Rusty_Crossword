...
use crossword::render::frame::{new_frame, Draw, Frame};
...
fn thread_render_loop(render_rx: Receiver<Message>) {
    ...
    loop {
        ...
        match message.stop_working {
            true => break,
            false => match message.frame {
                Some(frame) => {
                    frame_new = frame;
                    message_received = true;
                    render(&mut stdout, &frame_old, &frame_new, false, false);
                }
                None => {
...
fn component_user_write_answer(
            ...
            Event::Key(key) => match key.code {
                KeyCode::Char(ch) => {
                    ...
                    if to_long_user_input {
                        ...
                        crossword.set_instructions_to_user((
                            instructions.check_answer.clone(),
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let _result = stdout.execute(terminal::SetSize(
        WINDOW_SIZE_X as u16,
        WINDOW_SIZE_Y as u16,
    ));
                    ...
                    _ => {
                        ...
                        'keyword: for i in 1..=crossword.crossword_keywords.len() {
                                ...
                                == KeyCode::Char(i.to_string().chars().nth(0).unwrap())
                            {
                                ...
                                if component_user_write_answer(
                                    &mut crossword,
                                    &instructions,
                                    &mut audio,
                                    &response_to_user,
                        ...
                        if wrong_number_of_clue {
                            crossword
                                .response_to_user(response_to_user.not_clue_with_number.clone());
                            crossword.draw(&mut frame_new, start_strings);
                            let message = Message {
                                stop_working: false,
                                frame: Some(frame_new.clone()),
