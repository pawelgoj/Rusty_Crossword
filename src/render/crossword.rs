...
impl Crossword {
    fn arrange_indexes_in_order(
        crossword_keywords: Vec<CrosswordKeyword>,
    ) -> Vec<CrosswordKeyword> {
        let mut i: u8 = 0;
    ...
    fn find_position_of_keyword_by_intersect(
        mut question: CrosswordKeyword,
        question_in_crossword: &CrosswordKeyword,
        questions_taken: &Vec<CrosswordKeyword>,
    ) -> Option<CrosswordKeyword> {
        ...
        for ch in crossword_string_2.chars() {
            ...
            match index {
                Some(index) => match question_in_crossword.orientation {
                    Orientation::Horizontally => {
                        question.orientation = Orientation::Perpendicularly;
                        let x_min =
                            question_in_crossword.coord_start_x as i8 + i_in_crossword as i8;
                    ...
                    Orientation::Perpendicularly => {
                        ...
                        let y_min =
                            question_in_crossword.coord_start_y as i8 + i_in_crossword as i8;
    ...
    fn check_char_is_the_same(
        inserted: &CrosswordKeyword,
        in_crossword: &CrosswordKeyword,
        position_of_char_ins: u8,
        position_of_char_in_cross: u8,
    ) -> bool {
        let char_inserted = inserted
            .question
            .answer
            .chars()
            .nth(position_of_char_ins.into());
        match char_inserted {
            Some(_char_inserted) => {}
            None => return false,
        }

        let char_in_crossword = in_crossword
            .question
            .answer
            .chars()
            .nth(position_of_char_in_cross.into());
    ...
    fn check_collision(inserted: &CrosswordKeyword, in_crossword: &CrosswordKeyword) -> bool {
        match inserted.orientation {
            Orientation::Horizontally => match in_crossword.orientation {
                Orientation::Horizontally => {
                    let inserted_x_max = inserted.coord_start_x + inserted.len_of_keyword;
                    let in_crossword_x_max =
                        in_crossword.coord_start_x + in_crossword.len_of_keyword;
                ...
                Orientation::Perpendicularly => {
                    let inserted_x_max = inserted.coord_start_x + inserted.len_of_keyword;
                    let in_crossword_y_max =
                        in_crossword.coord_start_y + in_crossword.len_of_keyword;
                    ...
                    {
                        let position_of_char_in_cross =
                            inserted.coord_start_y as i8 - in_crossword.coord_start_y as i8;
                        let position_of_char_ins =
                            in_crossword.coord_start_x as i8 - inserted.coord_start_x as i8;

                        if position_of_char_in_cross < 0 || position_of_char_ins < 0 {
                            return true;
                        }

                        let the_same_char = Crossword::check_char_is_the_same(
                            &inserted,
                            &in_crossword,
                            position_of_char_ins as u8,
                            position_of_char_in_cross as u8,
                        );
            ...
            Orientation::Perpendicularly => match in_crossword.orientation {
                Orientation::Horizontally => {
                    let in_crossword_x_max =
                        in_crossword.coord_start_x + in_crossword.len_of_keyword;
                    ...
                    {
                        let position_of_char_in_cross =
                            inserted.coord_start_x as i8 - in_crossword.coord_start_x as i8;

                        let position_of_char_ins =
                            in_crossword.coord_start_y as i8 - inserted.coord_start_y as i8;
    ...
    pub fn new(questions: Vec<Question>) -> Self {
        ...
        for question in questions {
            ...
            let crossword_keyword =
                Crossword::determine_position(crossword_keyword, first, &questions_taken);

            match crossword_keyword {
                None => {}
                Some(crossword_keyword) => {
                    questions_taken.push(crossword_keyword);
                }
            }
            first = false;
        }
        ...
        Self {
            crossword_keywords: questions_taken,
            response_to_user: None,
            instructions_to_user: None,
        ...
        for keyword in &mut self.crossword_keywords {
            if keyword.question.id == question_id as u64 {
                match keyword.user_input {
                    Some(ref mut input) => {
                        input.push_str(&char.to_string());
                    }
                    None => {
                        keyword.user_input = Some(char.to_string());
                    }
                }
    ...
    pub fn draw(&self, frame: &mut super::frame::Frame, strings_start: u8) {
        ...
        for key_word in crossword_keywords {
            ...
            for i in 0..key_word.len_of_keyword {
                insert_char_in_crossword(
                    &mut positions_with_leters,
                    &key_word.orientation,
                    i,
                    col,
                    row,
                    frame,
        ...
        match &self.response_to_user {
            Some(ref response) => {
                questions_out.push_str(&response);
                questions_out.push_str(&"\n");
            }
            None => {}
        }
        ...
        let instructions = match self.instructions_to_user {
            Some(ref instruction) => {
                ...
                let mut line_1 =
                    complete_with_spaces(line_1.0, (WINDOW_SIZE_X + line_1.1) as usize);
                let line_2 = complete_with_spaces(line_2.0, (WINDOW_SIZE_X + line_2.1) as usize);
        ...
        fn insert_char_in_crossword(
            positions_with_leters: &mut HashSet<(u8, u8)>,
            orientation: &Orientation,
            i: u8,
            mut col: u8,
            mut row: u8,
            frame: &mut Frame,
            key_word: &CrosswordKeyword,
            char: &str,
            user_input: &String,
        ) {
            let col_id;
...
mod test_of_crossword {
    ...
    fn test_mocks(
        start_point_insert: (u8, u8),
        orientation_insert: Orientation,
        start_point_in_cross: (u8, u8),
        orientation_in_crossword: Orientation,
    ...
    fn test_find_position_of_keyword_by_intersect() {
        ...
        //When
        let result =
            Crossword::find_position_of_keyword_by_intersect(mocks.1, &mocks.0, &positions_taken);
    ...
    fn test_check_char() {
        ...
        //When
        let result =
            super::Crossword::check_char_is_the_same(&tuple_keywords.0, &tuple_keywords.1, 1, 2);
        //Then
        assert!(result);
        //Wen other chart
        let result =
            super::Crossword::check_char_is_the_same(&tuple_keywords.0, &tuple_keywords.1, 2, 2);
        assert!(!result);
    }
}
